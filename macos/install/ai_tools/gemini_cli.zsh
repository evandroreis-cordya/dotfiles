...
if [[ -f "$SHELL_PROFILE" ]]; then
    if ! grep -q "$GOOGLE_AI_CLI_DIR" "$SHELL_PROFILE"; then
        echo "" >> "$SHELL_PROFILE"
        echo "# Add local bin to PATH for Google AI CLI tools" >> "$SHELL_PROFILE"
        echo "export PATH=\"\$PATH:$GOOGLE_AI_CLI_DIR\"" >> "$SHELL_PROFILE"
        print_success "Added local bin to PATH in $SHELL_PROFILE"
    else
        print_success "Local bin already in PATH in $SHELL_PROFILE"
    fi
fi

# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Google AI Environment Variables
GOOGLE_AI_ENV_FILE="$GEMINI_CONFIG_DIR/env.sh"

cat > "$GOOGLE_AI_ENV_FILE" << 'EOF'
#!/bin/bash
# Google AI Environment Variables
...
chmod +x "$GOOGLE_AI_ENV_FILE"
print_success "Created Google AI environment variables file: $GOOGLE_AI_ENV_FILE"
...
export GEMINI_CONFIG_FILE="$GEMINI_CONFIG_FILE"
export GOOGLE_AI_SCRIPTS_DIR="$GOOGLE_AI_SCRIPTS_DIR"
export GOOGLE_AI_TEMPLATES_DIR="$GOOGLE_AI_TEMPLATES_DIR"

# Add environment variables to shell profile
if [[ -f "$SHELL_PROFILE" ]]; then
    if ! grep -q "GEMINI_CONFIG_DIR" "$SHELL_PROFILE"; then
        echo "" >> "$SHELL_PROFILE"
        echo "# Google AI and Gemini configuration" >> "$SHELL_PROFILE"
        echo "export GEMINI_CONFIG_DIR=\"$GEMINI_CONFIG_DIR\"" >> "$SHELL_PROFILE"
        echo "export GEMINI_CONFIG_FILE=\"$GEMINI_CONFIG_FILE\"" >> "$SHELL_PROFILE"
        echo "export GOOGLE_AI_SCRIPTS_DIR=\"$GOOGLE_AI_SCRIPTS_DIR\"" >> "$SHELL_PROFILE"
        echo "export GOOGLE_AI_TEMPLATES_DIR=\"$GOOGLE_AI_TEMPLATES_DIR\"" >> "$SHELL_PROFILE"
        echo "source \"$GOOGLE_AI_ENV_FILE\"" >> "$SHELL_PROFILE"
        print_success "Added Google AI environment variables to $SHELL_PROFILE"
    else
        print_success "Google AI environment variables already present in $SHELL_PROFILE"
    fi
fi
